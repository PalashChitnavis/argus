...
def _node_status(node: Node) -> str:
    ...
    if last_seen.tzinfo is None:
        # Some DB drivers (e.g. SQLite) return naive datetimes even for
        # DateTime(timezone=True) columns. Postgres returns tz-aware values
        # correctly, but we guard here so this endpoint never 500s regardless
        # of backend/driver quirks.
...
def get_node_dashboard(node_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        # Serialize through the Pydantic response models explicitly. Returning
        # raw ORM objects here (as this endpoint previously did) drops every
        # relationship field — connections, recent_logs, auth_events, packages,
        # interfaces, dns_servers, routing_table — because this route has no
        # response_model telling FastAPI how to walk those relationships.
        "startup_data": StartupDataResponse.model_validate(startup_data) if startup_data else None,
        "one_minute_data": OneMinuteDataResponse.model_validate(one_minute) if one_minute else None,
        "five_minute_data": FiveMinuteDataResponse.model_validate(five_minute) if five_minute else None,
        "thirty_minute_data": ThirtyMinuteDataResponse.model_validate(thirty_minute) if thirty_minute else None,
        "daily_data": DailyDataResponse.model_validate(daily_data) if daily_data else None,
    }